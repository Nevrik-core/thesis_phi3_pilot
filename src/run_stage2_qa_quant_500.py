...
def stop_all_stage2_models() -> None:
    model_names = {
        ...
        for key in STAGE2_QA_QUANT_MODEL_KEYS
    }
...
def save_manifest(output_dir, experiment_version: str) -> None:
    ...
    for i, key in enumerate(STAGE2_QA_QUANT_MODEL_KEYS, start=1):
    ...
    path = output_dir / f"stage2_qa_quant_manifest_{experiment_version}.csv"
...
def build_output_paths(output_dir, cfg: dict, experiment_version: str) -> tuple:
    details_path = (
        output_dir
        / f"stage2_qa_details_{cfg['backend_name']}_ua{UA_QA_SUBSET_SIZE}_en{EN_QA_SUBSET_SIZE}_{experiment_version}.csv"
    )
    summary_path = (
        output_dir
        / f"stage2_qa_summary_{cfg['backend_name']}_ua{UA_QA_SUBSET_SIZE}_en{EN_QA_SUBSET_SIZE}_{experiment_version}.csv"
    )
...
def run_model(model_key: str, output_dir, experiment_version: str) -> None:
    ...
    print("\n" + "#" * 100)
    print(f"# STAGE2 QA QUANT: {model_key}")
    ...
    ua_df, ua_summary = run_eval_for_dataset(
        dataset=ua_ds,
        cfg=cfg,
        lang="uk",
    ...
    summary = pd.DataFrame([ua_summary, en_summary])
...
def main() -> None:
    experiment_version = os.getenv("EXPERIMENT_VERSION", "stage2_qa_quant_200_v1")
    ...
    print(f"Models: {len(STAGE2_QA_QUANT_MODEL_KEYS)}")
    print(f"UA QA size: {UA_QA_SUBSET_SIZE}")
    print(f"EN QA size: {EN_QA_SUBSET_SIZE}")
    print(f"Cold start Ollama: {os.getenv('STAGE2_COLD_START_OLLAMA', '1')}")
    print(f"Pull models: {os.getenv('STAGE2_NO_PULL', '1') != '1'}")

    save_manifest(output_dir, experiment_version)

    for model_key in STAGE2_QA_QUANT_MODEL_KEYS:
